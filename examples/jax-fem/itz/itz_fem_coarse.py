...
geom_blocks_flat = geom_blocks.transpose(0, 2, 4, 1, 3, 5).reshape(Cx, Cy, Cz, POOL**3)
...
for ix in range(Cx):
    for iy in range(Cy):
        for iz in range(Cz):
            ...
            solid = solid[solid >= 0]
            if len(solid) > 0:
                dominant_phase[ix, iy, iz] = int(mode(solid, keepdims=False).mode)
            else:
...
if n_components > 1:
    ...
    print(f"Single connected component, {int(solid_mask_3d.sum())} solid elements")
