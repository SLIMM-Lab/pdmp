...
if n_components > 1:
    ...
    print(
        f"Single connected component, {int(solid_mask_3d.sum())} solid voxels")
...
eps_cell = onp.array(np.sum(epsilon * w[:, :, None, None],
                            axis=1))  # (nc, 3, 3)
sigma_cell = onp.array(np.sum(sigma * w[:, :, None, None],
                              axis=1))  # (nc, 3, 3)
