...
_flags = {a.lstrip('-').split('=')[0]: (a.split('=')[1] if '=' in a else None)
...
_FACE_DISPLAY = {
    'face_x_min': (lambda sm, cx, cy, cz: sm[0,    :, :], 1, 2, 'y (µm)', 'z (µm)'),
    'face_x_max': (lambda sm, cx, cy, cz: sm[cx-1, :, :], 1, 2, 'y (µm)', 'z (µm)'),
    'face_y_min': (lambda sm, cx, cy, cz: sm[:,  0,  :], 0, 2, 'x (µm)', 'z (µm)'),
    'face_y_max': (lambda sm, cx, cy, cz: sm[:, cy-1, :], 0, 2, 'x (µm)', 'z (µm)'),
    'face_z_max': (lambda sm, cx, cy, cz: sm[:, :, cz-1], 0, 1, 'x (µm)', 'y (µm)'),
}
_FACE_COLORS = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']


def _plot_sensors(sensor_specs, solid_mask_3d,
                  cx, cy, cz, elem_size,
                  domain_x, domain_y, domain_z, out_dir):
    ...
    corners = onp.array([
        [0, 0, 0], [domain_x, 0, 0], [domain_x, domain_y, 0], [0, domain_y, 0],
    ...
    for name, pts in sensor_pts.items():
        ...
        ax.scatter(pts[:, 0], pts[:, 1], pts[:, 2],
                   s=40, color=c, label=name, zorder=5,
                   edgecolors='k', linewidths=0.4)

    ax.set_xlabel('x (µm)', labelpad=4)
    ax.set_ylabel('y (µm)', labelpad=4)
    ax.set_zlabel('z (µm)', labelpad=4)
    ax.set_title(f'{sum(len(p) for p in sensor_pts.values())} sensors, POOL={POOL}')
    ...
    fig2, axes = plt.subplots(1, n_panels, figsize=(3.5 * n_panels, 4),
    ...
    for ax2, fname in zip(axes, present):
        ...
        ax2.imshow(mask.T, origin='lower', extent=extents[fname],
                   cmap='Greys', vmin=0, vmax=1, aspect='auto', alpha=0.5)

        # Selected sensors
        if fname in sensor_pts:
            spts = sensor_pts[fname]
            ax2.scatter(spts[:, hcoord], spts[:, vcoord],
                        s=50, color=color_map.get(fname, 'C5'),
    ...
    fig2.suptitle('Sensor placement on boundary faces (dark = solid, light = void)',
                  fontsize=9)
...
def _build_face_table(solid_mask_3d, cx, cy, cz, es,
                      domain_x, domain_y, domain_z):
    """Return ordered dict {name: (loc_fn, layer2d, dim0, dim1, make_pt)}."""
    return {
        'face_x_min': ('side_faces', solid_mask_3d[0,    :, :],
                       domain_y, domain_z, cy, cz,
                       lambda u, v: onp.array([0.0,      u, v])),
        'face_x_max': ('side_faces', solid_mask_3d[cx-1, :, :],
                       domain_y, domain_z, cy, cz,
                       lambda u, v: onp.array([domain_x, u, v])),
        'face_y_min': ('side_faces', solid_mask_3d[:, 0,    :],
                       domain_x, domain_z, cx, cz,
                       lambda u, v: onp.array([u, 0.0,      v])),
        'face_y_max': ('side_faces', solid_mask_3d[:, cy-1, :],
                       domain_x, domain_z, cx, cz,
                       lambda u, v: onp.array([u, domain_y, v])),
        'face_z_max': ('top_face',   solid_mask_3d[:, :, cz-1],
                       domain_x, domain_y, cx, cy,
                       lambda u, v: onp.array([u, v, domain_z])),
    }


def _random_sensors(solid_mask_3d, cx, cy, cz, es,
                    domain_x, domain_y, domain_z, n_sensors, rng):
    ...
    """
    faces = _build_face_table(solid_mask_3d, cx, cy, cz, es,
                              domain_x, domain_y, domain_z)

    # Count solid elements per face for proportional allocation
    solid_counts = {name: int(info[1].sum()) for name, info in faces.items()
                    if info[1].any()}
    ...
    for name in sorted(face_names, key=lambda n: raw[n] - allocated[n], reverse=True)[:shortfall]:
        allocated[name] += 1
...
with open(sensors_path, 'w') as f:
    yaml.dump(numpy_to_yaml({'sensors': sensor_specs}), f, default_flow_style=None, sort_keys=False)
...
noise = onp.random.default_rng(SEED).normal(0.0, NOISE_STD, size=observations.shape)
...
if PLOT:
    _plot_sensors(sensor_specs, solid_mask_3d,
                  Cx, Cy, Cz, elem_size, domain_x, domain_y, domain_z, OUT_DIR)
