...
if __name__ == '__main__':
    ...
    if len(model.sensor_interpolants) == 2:
        ...
        same_weights = np.allclose(np.sort(s0['weights'][0]),
                                   np.sort(s1['weights'][0]))
        ...
        if not same_nodes:
            ...
            print(
                "      This means they can give different readings even at the same E."
            )
    ...
    if len(readings) == 2:
        ...
        if np.allclose(diff, 0, atol=1e-8):
            ...
            print(
                "  ✗  Sensors disagree — likely due to different interpolating faces."
            )
    ...
    print(
        f"  Test point: {node_pt}  (should be on both side_faces and top_face)"
    )
    ...
    try:
        node_interps = build_sensor_interpolants(fe, node_sensors,
                                                 location_fn_map)
        ...
        if len(node_readings) == 2:
            diff_node = np.asarray(node_readings[0]['u']) - np.asarray(
                node_readings[1]['u'])
            print(
                f"  Node-point difference: {diff_node}  (max={np.max(np.abs(diff_node)):.2e})"
            )
    ...
    print(
        f"  test_pt y=0.49 — nearest nodes: "
        f"{np.arange(0, d_y + h/2, h)[np.argsort(np.abs(np.arange(0, d_y + h/2, h) - 0.49))[:2]]}"
    )
    print(
        "  y=0.49 lies INSIDE a face element (not on a node), so interpolation is used."
    )
    print()
    print(
        "  Both sensors find the shared edge (x=0, z=d_z) and its two bounding nodes."
    )
    print("  side_faces picks the x=0 face; top_face picks the z=d_z face.")
    print(
        "  They share nodes 30 and 31, but differ in their 4th node (x≠0 vs z≠d_z)."
    )
    print(
        "  The weights on the *shared* nodes are identical, so the non-shared nodes"
    )
    print(
        "  receive zero weight — both sensors effectively interpolate identically."
    )
    print()
    print(
        "  The tiny residual difference (~1e-9) is pure floating-point rounding;"
    )
