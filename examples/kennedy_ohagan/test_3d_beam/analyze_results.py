...
PARAM_NAMES = [
    r"$\theta_\rho$", r"$\theta_l$", r"$\log\sigma^2_\delta$",
    r"$\log\sigma^2_\varepsilon$", r"$\log\rho_\mathrm{KO}$"
]
...
def _load_samples(directory: str, label: str):
    ...
    print(
        f"{label}: loaded {samples.shape[0]} samples, dim={samples.shape[1]}")
    return samples
...
def main():
    ...
    if gt is not None:
        ...
        for i, name in enumerate(PARAM_NAMES):
            print(
                f"  {name:35s} = {gt_latent[i]:.4f}  (physical: {gt[i]:.4f})")
    else:
        gt_latent = None
    ...
    rwm_no_ko_samples = (None if args.no_ko else _load_samples(
        RWM_NO_KO_DIR, "RWM (no K&O)"))
