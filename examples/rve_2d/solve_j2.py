...
def plot_field(mesh, facecolors, phys_tags, L, R, cx, cy, title, label, cmap,
               fig_path):
...
def plot_results(mesh,
    ...
    plot_field(mesh, vm, phys_tags, L, R, cx, cy, "von Mises stress",
...
def main():
    ...
    for k in range(1, n_steps + 1):
        ...
        eps_p_q, alpha_q = problem.update_int_vars_gp(sol_list[0],
    ...
    # Stress-strain curve
    plot_stress_strain_curve(eps_history, sigma_history,
    ...
    eps_p_cell = np.sum(np.array(eps_p_q) * w[:, :, None, None],
                        axis=1)  # (nc, 2, 2)
