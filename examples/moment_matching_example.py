...
laplace_gaussian = MultivariateNormal(map_point,
                                      laplace_cov,
...
print(
    f"Laplace std (dim 0)  : {np.sqrt(laplace_cov[0,0]):.4f}  (true: 1.0000)")
...
chain, rate = _mrw_samples(target.log_density,
                           dim=2,
                           n_samples=5000,
                           step=0.4,
                           seed=42)
...
for i, name in enumerate(output_names):
    print(
        f"  {name:<22}  {mu_mc[i]:>8.4f}  {mu_ut[i]:>8.4f}  {mu_lin[i]:>8.4f}")

print(f"\n{'Quantity':<24}  {'MC std':>8}  {'UT std':>8}  {'Lin std':>8}")
print("-" * 56)
for i, name in enumerate(output_names):
    print(
        f"  {name:<22}  {std_mc[i]:>8.4f}  {std_ut[i]:>8.4f}  {std_lin[i]:>8.4f}"
    )
...
# 2-D scatter
ax = axes[0]
ax.scatter(mc_samples[:, 0],
           mc_samples[:, 1],
           s=3,
           alpha=0.2,
           color='steelblue',
           label='MC samples')
ax.scatter(laplace_samples[:, 0],
           laplace_samples[:, 1],
           s=3,
           alpha=0.1,
           color='tomato',
           label='Laplace samples')
_confidence_ellipse(ax,
                    map_point,
                    laplace_cov,
                    n_std=2,
                    edgecolor='tomato',
...
for ax_i, dim, xlbl in zip(axes[1:], [0, 1], [
    ...
    lap_pdf = norm.pdf(grid,
                       loc=map_point[dim],
                       scale=np.sqrt(laplace_cov[dim, dim]))
    ax.hist(mc_samples[:, dim],
            bins=50,
            density=True,
            alpha=0.4,
            color='steelblue',
...
fig2.suptitle("Example 2 — Forward UQ: UT vs linearization vs MC", fontsize=11)
...
for k, (method, means, stds,
        color) in enumerate(zip(methods, means_all, stds_all, colors)):
    ax.bar(x_pos + (k - 1) * width,
           means,
           width,
           yerr=stds,
           capsize=4,
